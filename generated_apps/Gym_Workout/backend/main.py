...
from models import Gym_workout

Base.metadata.create_all(bind=engine)

app = FastAPI(title="Gym_Workout API")
...
def health():
    return {"status": "ok", "app": "Gym_Workout"}
...
def read_items(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return db.query(Gym_workout).offset(skip).limit(limit).all()


@app.get("/items/{item_id}", response_model=ItemOut, tags=["items"])
def read_item(item_id: int, db: Session = Depends(get_db)):
    obj = db.query(Gym_workout).filter(Gym_workout.id == item_id).first()
    if not obj:
