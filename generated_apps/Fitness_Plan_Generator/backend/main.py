...
from models import Fitness_plan_generator
...
app = FastAPI(title="Fitness_Plan_Generator API")
...
async def on_startup():
    logger.info("Fitness_Plan_Generator starting up — frontend: %s", FRONTEND_DIR)


@app.on_event("shutdown")
async def on_shutdown():
    logger.info("Fitness_Plan_Generator shutting down")
...
def health():
    return {"status": "ok", "app": "Fitness_Plan_Generator"}
...
def create_item(item: ItemCreate, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        obj = Fitness_plan_generator(
        exercise=_safe_str(item.exercise),
        sets=_safe_int(item.sets),
        reps=_safe_int(item.reps),
        weight=_safe_float(item.weight),
        workout_date=_safe_date(item.workout_date),
        completed=_safe_bool(item.completed),
        )
        db.add(obj)
        db.commit()
        db.refresh(obj)
...
def read_items(request: Request, skip: int = 0, limit: int = 500, db: Session = Depends(get_db)):
    ...
    try:
        return (
            db.query(Fitness_plan_generator)
            .order_by(Fitness_plan_generator.id.desc())
...
def read_item(item_id: int, request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    obj = db.query(Fitness_plan_generator).filter(Fitness_plan_generator.id == item_id).first()
    if not obj:
...
def update_item(item_id: int, item: ItemUpdate, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        if item.exercise is not None:
            obj.exercise = _safe_str(item.exercise)
        if item.sets is not None:
            obj.sets = _safe_int(item.sets)
        if item.reps is not None:
            obj.reps = _safe_int(item.reps)
        if item.weight is not None:
            obj.weight = _safe_float(item.weight)
        if item.workout_date is not None:
            obj.workout_date = _safe_date(item.workout_date)
        if item.completed is not None:
            obj.completed = _safe_bool(item.completed)
        db.commit()
        db.refresh(obj)
        return obj
    except Exception as exc:
...
def export_json(request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    try:
        items = db.query(Fitness_plan_generator).order_by(Fitness_plan_generator.id.desc()).all()
