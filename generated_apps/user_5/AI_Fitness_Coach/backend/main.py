...
from models import Ai_fitness_coach
...
app = FastAPI(title="AI_Fitness_Coach API")
...
async def on_startup():
    logger.info("AI_Fitness_Coach starting up — frontend: %s", FRONTEND_DIR)


@app.on_event("shutdown")
async def on_shutdown():
    logger.info("AI_Fitness_Coach shutting down")
...
def health():
    return {"status": "ok", "app": "AI_Fitness_Coach"}
...
def read_items(request: Request, skip: int = 0, limit: int = 500, db: Session = Depends(get_db)):
    ...
    try:
        return (
            db.query(Ai_fitness_coach)
            .order_by(Ai_fitness_coach.id.desc())
...
def read_item(item_id: int, request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    obj = db.query(Ai_fitness_coach).filter(Ai_fitness_coach.id == item_id).first()
    if not obj:
...
def export_json(request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    try:
        items = db.query(Ai_fitness_coach).order_by(Ai_fitness_coach.id.desc()).all()
