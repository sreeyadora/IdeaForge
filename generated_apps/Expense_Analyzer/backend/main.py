...
from models import Expense_analyzer
...
app = FastAPI(title="Expense_Analyzer API")
...
async def on_startup():
    logger.info("Expense_Analyzer starting up — frontend: %s", FRONTEND_DIR)


@app.on_event("shutdown")
async def on_shutdown():
    logger.info("Expense_Analyzer shutting down")
...
def health():
    return {"status": "ok", "app": "Expense_Analyzer"}
...
def read_items(request: Request, skip: int = 0, limit: int = 500, db: Session = Depends(get_db)):
    ...
    try:
        return (
            db.query(Expense_analyzer)
            .order_by(Expense_analyzer.id.desc())
...
def read_item(item_id: int, request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    obj = db.query(Expense_analyzer).filter(Expense_analyzer.id == item_id).first()
    if not obj:
...
def update_item(item_id: int, item: ItemUpdate, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if item.expense_date is not None:
            obj.expense_date = _safe_date(item.expense_date)
...
def export_json(request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    try:
        items = db.query(Expense_analyzer).order_by(Expense_analyzer.id.desc()).all()
