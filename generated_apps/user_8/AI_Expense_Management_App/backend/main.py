...
from models import Ai_expense_management_app
...
app = FastAPI(title="AI_Expense_Management_App API")
...
async def on_startup():
    logger.info("AI_Expense_Management_App starting up — frontend: %s", FRONTEND_DIR)


@app.on_event("shutdown")
async def on_shutdown():
    logger.info("AI_Expense_Management_App shutting down")
...
def health():
    return {"status": "ok", "app": "AI_Expense_Management_App"}
...
def read_items(request: Request, skip: int = 0, limit: int = 500, db: Session = Depends(get_db)):
    ...
    try:
        return (
            db.query(Ai_expense_management_app)
            .order_by(Ai_expense_management_app.id.desc())
...
def read_item(item_id: int, request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    obj = db.query(Ai_expense_management_app).filter(Ai_expense_management_app.id == item_id).first()
    if not obj:
...
def update_item(item_id: int, item: ItemUpdate, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if item.description is not None:
            obj.description = _safe_str(item.description)
        db.commit()
        db.refresh(obj)
        return obj
    except Exception as exc:
...
def export_json(request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    try:
        items = db.query(Ai_expense_management_app).order_by(Ai_expense_management_app.id.desc()).all()
