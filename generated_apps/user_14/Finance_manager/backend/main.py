...
from models import Finance_manager
...
app = FastAPI(title="Finance_manager API")
...
async def on_startup():
    logger.info("Finance_manager starting up — frontend: %s", FRONTEND_DIR)


@app.on_event("shutdown")
async def on_shutdown():
    logger.info("Finance_manager shutting down")
...
def health():
    return {"status": "ok", "app": "Finance_manager"}
...
def read_items(request: Request, skip: int = 0, limit: int = 500, db: Session = Depends(get_db)):
    ...
    try:
        return (
            db.query(Finance_manager)
            .order_by(Finance_manager.id.desc())
...
def read_item(item_id: int, request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    obj = db.query(Finance_manager).filter(Finance_manager.id == item_id).first()
    if not obj:
...
def export_json(request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    try:
        items = db.query(Finance_manager).order_by(Finance_manager.id.desc()).all()
