...
def create_item(item: ItemCreate, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        expense_date=item.expense_date if item.expense_date is not None else "",
        is_recurring=item.is_recurring if item.is_recurring is not None else False,
        )
        db.add(obj)
        db.commit()
        db.refresh(obj)
...
def update_item(item_id: int, item: ItemUpdate, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if item.expense_date is not None:
            obj.expense_date = item.expense_date
        if item.is_recurring is not None:
            obj.is_recurring = bool(item.is_recurring)
        db.commit()
        db.refresh(obj)
        return obj
    except Exception as exc:
