...
async def on_startup():
    logger.info("GymTracker starting up — frontend: %s", FRONTEND_DIR)


@app.on_event("shutdown")
async def on_shutdown():
    logger.info("GymTracker shutting down")
...
def health():
    return {"status": "ok", "app": "GymTracker"}
...
def read_item(item_id: int, request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    obj = db.query(Gymtracker).filter(Gymtracker.id == item_id).first()
    if not obj:
...
def export_json(request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    try:
        items = db.query(Gymtracker).order_by(Gymtracker.id.desc()).all()
