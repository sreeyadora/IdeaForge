...
from models import Smart_study_planner
...
app = FastAPI(title="Smart_Study_Planner API")
...
async def on_startup():
    logger.info("Smart_Study_Planner starting up — frontend: %s", FRONTEND_DIR)


@app.on_event("shutdown")
async def on_shutdown():
    logger.info("Smart_Study_Planner shutting down")
...
def health():
    return {"status": "ok", "app": "Smart_Study_Planner"}
...
def create_item(item: ItemCreate, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        obj = Smart_study_planner(
        subject=_safe_str(item.subject),
        topic=_safe_str(item.topic),
        duration_minutes=_safe_int(item.duration_minutes),
        study_date=_safe_date(item.study_date),
...
def read_items(request: Request, skip: int = 0, limit: int = 500, db: Session = Depends(get_db)):
    ...
    try:
        return (
            db.query(Smart_study_planner)
            .order_by(Smart_study_planner.id.desc())
...
def read_item(item_id: int, request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    obj = db.query(Smart_study_planner).filter(Smart_study_planner.id == item_id).first()
    if not obj:
...
def update_item(item_id: int, item: ItemUpdate, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        if item.subject is not None:
            obj.subject = _safe_str(item.subject)
        if item.topic is not None:
            obj.topic = _safe_str(item.topic)
        if item.duration_minutes is not None:
            obj.duration_minutes = _safe_int(item.duration_minutes)
        if item.study_date is not None:
            obj.study_date = _safe_date(item.study_date)
        if item.completed is not None:
            obj.completed = _safe_bool(item.completed)
        if item.notes is not None:
            obj.notes = _safe_str(item.notes)
        db.commit()
        db.refresh(obj)
        return obj
    except Exception as exc:
...
def export_json(request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    try:
        items = db.query(Smart_study_planner).order_by(Smart_study_planner.id.desc()).all()
