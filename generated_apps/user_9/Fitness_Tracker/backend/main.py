...
from models import Fitness_tracker
...
app = FastAPI(title="Fitness_Tracker API")
...
async def on_startup():
    logger.info("Fitness_Tracker starting up — frontend: %s", FRONTEND_DIR)


@app.on_event("shutdown")
async def on_shutdown():
    logger.info("Fitness_Tracker shutting down")
...
def health():
    return {"status": "ok", "app": "Fitness_Tracker"}
...
def read_items(request: Request, skip: int = 0, limit: int = 500, db: Session = Depends(get_db)):
    ...
    try:
        return (
            db.query(Fitness_tracker)
            .order_by(Fitness_tracker.id.desc())
...
def read_item(item_id: int, request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    obj = db.query(Fitness_tracker).filter(Fitness_tracker.id == item_id).first()
    if not obj:
...
def export_json(request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    try:
        items = db.query(Fitness_tracker).order_by(Fitness_tracker.id.desc()).all()
