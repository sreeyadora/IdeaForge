...
from models import Expence_tracker
...
app = FastAPI(title="expence_tracker API")
...
async def on_startup():
    logger.info("expence_tracker starting up — frontend: %s", FRONTEND_DIR)


@app.on_event("shutdown")
async def on_shutdown():
    logger.info("expence_tracker shutting down")
...
def health():
    return {"status": "ok", "app": "expence_tracker"}
...
def read_items(request: Request, skip: int = 0, limit: int = 500, db: Session = Depends(get_db)):
    ...
    try:
        return (
            db.query(Expence_tracker)
            .order_by(Expence_tracker.id.desc())
...
def read_item(item_id: int, request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    obj = db.query(Expence_tracker).filter(Expence_tracker.id == item_id).first()
    if not obj:
...
def export_json(request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    try:
        items = db.query(Expence_tracker).order_by(Expence_tracker.id.desc()).all()
