...
from models import Travel_itinerary_generator
...
app = FastAPI(title="Travel_Itinerary_Generator API")
...
async def on_startup():
    logger.info("Travel_Itinerary_Generator starting up — frontend: %s", FRONTEND_DIR)


@app.on_event("shutdown")
async def on_shutdown():
    logger.info("Travel_Itinerary_Generator shutting down")
...
def health():
    return {"status": "ok", "app": "Travel_Itinerary_Generator"}
...
def create_item(item: ItemCreate, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        obj = Travel_itinerary_generator(
        title=_safe_str(item.title),
        amount=_safe_float(item.amount),
        category=_safe_str(item.category),
        date=_safe_date(item.date),
        is_recurring=_safe_bool(item.is_recurring),
        )
        db.add(obj)
        db.commit()
        db.refresh(obj)
...
def read_items(request: Request, skip: int = 0, limit: int = 500, db: Session = Depends(get_db)):
    ...
    try:
        return (
            db.query(Travel_itinerary_generator)
            .order_by(Travel_itinerary_generator.id.desc())
...
def read_item(item_id: int, request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    obj = db.query(Travel_itinerary_generator).filter(Travel_itinerary_generator.id == item_id).first()
    if not obj:
...
def update_item(item_id: int, item: ItemUpdate, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        if item.title is not None:
            obj.title = _safe_str(item.title)
        if item.amount is not None:
            obj.amount = _safe_float(item.amount)
        if item.category is not None:
            obj.category = _safe_str(item.category)
        if item.date is not None:
            obj.date = _safe_date(item.date)
        if item.is_recurring is not None:
            obj.is_recurring = _safe_bool(item.is_recurring)
        db.commit()
        db.refresh(obj)
        return obj
    except Exception as exc:
...
def export_json(request: Request, db: Session = Depends(get_db)):
    _require_user(request)
    try:
        items = db.query(Travel_itinerary_generator).order_by(Travel_itinerary_generator.id.desc()).all()
