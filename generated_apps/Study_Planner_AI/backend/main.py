...
from models import Study_planner_ai

Base.metadata.create_all(bind=engine)

app = FastAPI(title="Study_Planner_AI API")
...
def health():
    return {"status": "ok", "app": "Study_Planner_AI"}
...
def read_items(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return db.query(Study_planner_ai).offset(skip).limit(limit).all()


@app.get("/items/{item_id}", response_model=ItemOut, tags=["items"])
def read_item(item_id: int, db: Session = Depends(get_db)):
    obj = db.query(Study_planner_ai).filter(Study_planner_ai.id == item_id).first()
    if not obj:
